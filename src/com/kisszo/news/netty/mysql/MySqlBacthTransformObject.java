...
public class MySqlBacthTransformObject extends AbstractMysqlDatabaseManager{
	...
	private void createVertex(Object[] val) throws ClassNotFoundException, IllegalAccessException, IllegalArgumentException, InvocationTargetException, SQLException, ParseException{
		...
		String clazzName = val[0].getClass().getName();
		AbstractMysqlPojo[] bean = (AbstractMysqlPojo[]) val;
		Class clazz = Class.forName(clazzName);
		Connection conn = getConnection();
		String sql = buildSql(clazz,bean[0]);
		System.out.println("Build sql statement"+sql);
		PreparedStatement  preparedStatement = conn.prepareStatement(sql);
		for (AbstractMysqlPojo abstractMysqlPojo : bean) {
			abstractMysqlPojo.setPreparedStatement(preparedStatement);
			mapProperties(clazz,abstractMysqlPojo);
...
//			String sql = "INSERT IGNORE INTO "+bean.getClassName();
